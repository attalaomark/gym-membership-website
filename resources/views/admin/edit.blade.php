...
<main class="admin-dashboard">
    <div class="container">
        <div class="dashboard-card">
            ...
            <form action="{{ route('anggota.updateMember', $anggota->id_anggota) }}" method="POST">
                @csrf
                @method('PUT') <!-- Jika menggunakan metode PUT untuk update -->
                
                <div class="form-group">
                    <label for="nama_pertama">Nama Pertama:</label>
                    <input type="text" class="form-control" id="nama_pertama" name="nama_pertama" value="{{ $anggota->nama_pertama }}" required>
                </div>

                <div class="form-group">
                    <label for="nama_akhir">Nama Akhir:</label>
                    <input type="text" class="form-control" id="nama_akhir" name="nama_akhir" value="{{ $anggota->nama_akhir }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="membershipType" name="id_paket" required>
                        ...
                        @foreach($packages as $package)
                            <option value="{{ $package->id_paket }}" {{ $anggota->id_paket == $package->id_paket ? 'selected' : '' }}>
                                {{ $package->deskripsi }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="joinDate" name="tanggal_mulai" value="{{ $anggota->tanggal_mulai }}" required>
                </div>
